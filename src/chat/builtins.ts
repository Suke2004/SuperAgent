 ...
 * for a PDF could only describe one. These four are the smallest set that closes
 * that gap — write a file, render a PDF, read a page, read a resource — and they are
 * deliberately not a shell. There is no `run_command` here, and one is not planned:
 ...
 *
 * `write_file` and `create_pdf` are always offered: they write inside the app's own
 * directory, which is reversible and visible, and a document tool nobody had to
 * enable is the difference between the feature existing and the feature being found.
 * `fetch_url` is not, because it makes requests on the user's network at the
